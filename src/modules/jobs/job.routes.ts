...
jobRouter.get("/mine", authenticate, authorize(UserRole.RECRUITER), asyncHandler(async (req, res) => res.json({ success: true, jobs: await Job.findAll({ where: { recruiterId: req.user!.id }, include: [{ model: Company, as: "company" }], order: [["createdAt", "DESC"]] }) })));
jobRouter.get("/:id", asyncHandler(async (req, res, next) => { const job = await Job.findByPk(String(req.params.id), { include: [{ model: Company, as: "company" }] }); if (!job) return next(new AppError("Job not found", 404)); return res.json({ success: true, job }); }));
